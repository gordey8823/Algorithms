...
"""Вывод: В данном скрипте для оптимизации использовалась библиотека numpy 
это привело к сокрашению памяти в 2 раза"""
...
def func_2(nums):
    new_arr2 = array([i for i in range(len(nums)) if not nums[i] % 2])
    return new_arr2
